...
Sistema de sala para Carly’s Restobar, en Turbaco, Bolívar. Cubre el sitio
público, la toma de pedidos en el salón, la pantalla de cocina y la de barra, la
recepción de domicilios y para llevar, el cobro y el panel administrativo.

> **La casa tiene dos sedes, Turbaco y Arjona. Este sistema es el de Turbaco.**
> Arjona no está montada todavía; cuando lo esté va a ser otro despliegue de
> este mismo software, con su propia base y su propia caja, unido a este por un
> botón en el panel administrativo: ver [La otra sede](#la-otra-sede).

## Lo que está pendiente de confirmar

Estas cosas están puestas con un valor provisional para que el sistema arranque
y se pueda probar completo. Ninguna impide usarlo; todas hay que corregirlas
antes de abrirlo al público.

| Qué | Dónde está hoy | Cómo se corrige |
| --- | --- | --- |
| **La dirección del local** | «Carrera 27 con Calle 28», la esquina de la ficha de Google Maps (la casa no tiene placa a mano) | Desde `/recepcion/ajustes`, sin desplegar |
| **El horario** | Uno típico de restobar | Desde `/recepcion/ajustes`, sin desplegar |
| **Las coordenadas** | Las de la ficha de Google Maps (10.3392264, -75.4154494). Solo cambian si el local se muda | `src/compartido/config.ts` y `carlys.local` en `application.yml`. **Las dos a la vez**: si no coinciden, el mapa muestra un punto y el domicilio se cobra desde otro |
| **La carta** | 34 productos de ejemplo | Desde `/admin/carta`, sin desplegar |
| **El salón** | 23 mesas en cuatro zonas | Desde `/admin/configuracion` |
| **Los datos del RUT** | Dicen «POR DEFINIR» y salen así en el papel | `DATOS_FISCALES` en `src/compartido/config.ts`, copiando el RUT |
| **El logotipo** | Calcado a vector desde el avatar de Instagram (la casa no tiene el original): `public/logo.svg` y `public/emblema.svg` | Solo si aparece el archivo del diseñador: reemplazar los dos SVG y volver a exportar los PNG |
| **El dominio** | `carlysrestobar.co` es un supuesto: la casa no tiene dominio todavía | Al comprarlo, cambiar las dos etiquetas de imagen de `index.html` y `CARLYS_CORS_ORIGENES` |
| **Quién factura** | Nadie: el sistema no emite documentos fiscales | Decisión del contador. Ver [Facturación](FACTURACION-DIAN.md) |
...
- [Lo que está pendiente de confirmar](#lo-que-está-pendiente-de-confirmar)
- [La otra sede](#la-otra-sede)
...
Carly’s tiene dos locales, **Turbaco** y **Arjona**. Este repositorio es el de
Turbaco. El día que Arjona entre, va a ser **otro despliegue completamente
separado**: su propia base de datos, su carta, su personal, su caja y su
dominio.

```
    carlysrestobar.co                  arjona.carlysrestobar.co
 ┌──────────────┐                  ┌──────────────┐
 │  Público     │                  │  Público     │
 │  Salón       │                  │  Salón       │
 │  Admin  ─────┼───── salta ─────▶│  Admin       │
 └──────┬───────┘                  └──────┬───────┘
  BD carlys                         BD carlys_arjona
```

Separados, y no una sola base con una columna «sede», porque la venta de dos
locales en una sola base es un problema contable esperando el cierre de mes: son
dos cajas, dos inventarios y dos nóminas, y una consulta a la que se le olvide
filtrar no da un error sino una cifra creíble y equivocada. El precio es que el
código queda duplicado en dos repositorios: un arreglo hay que aplicarlo dos
veces, y la forma barata de hacerlo es `git cherry-pick`, no reescribirlo.

**Nada de esto está encendido hoy.** Todo lo que sigue en esta sección describe
un mecanismo que ya está programado y probado, y que se activa llenando
variables de entorno el día que exista la segunda sede. Con las variables
vacías —como están— el selector no se pinta y no se emite ni se acepta ningún
pase.

### El botón que los une

En `/admin` aparece arriba a la derecha un selector con el nombre de la sede en
la que se está; al desplegarlo, un enlace lleva **a la misma sección** del otro
local. Solo lo ve el administrador: a un cajero no le sirve, trabaja en un local
y el otro no lo conoce.

Se enciende con una variable en el frontend:

```bash
VITE_URL_SEDE_HERMANA=https://carlys-arjona-frontend.up.railway.app
```

Sin ella el selector **no se pinta**, que es la situación de hoy y también la de
desarrollo: un botón que lleva a una dirección muerta es peor que no tener
botón.

### La cuenta del dueño es una sola

El dueño tendría una cuenta de administrador en cada local, y serían dos
cuentas de verdad: cambiar la clave en Turbaco no la cambiaría en Arjona, y
habría que acordarse de hacerlo dos veces y de cuál era cuál.

Con esto, **el cambio que hace en un panel viaja al otro**. Se activa con la
dirección del backend de la otra sede:

```bash
CARLYS_CRUCE_API_HERMANA=https://carlys-arjona-backend.up.railway.app
```
...
esos dicen qué puede hacer *aquí*, y son decisión de cada sede: suspenderle el
acceso a un local no tiene por qué cerrarle el otro.
...
La llamada a la otra sede se hace en el controlador y no en el servicio, a propósito:
...
quién es el dueño, firmado con un secreto que comparten las dos sedes.
...
Se enciende con `CARLYS_CRUCE_SECRETO`, **el mismo valor en las dos sedes** y
distinto del de las sesiones. Sin esa variable el cruce queda apagado en los dos
sentidos y el selector sigue saltando: el otro sistema pedirá la clave.

Compartir el secreto de las sesiones habría sido más simple y habría significado
que **cualquier** token de una sede vale en la otra. Con un secreto propio,
...
Nada de esto se activa hasta que exista la segunda sede y las dos tengan la
misma variable.
...
CARLYS_JWT_SECRETO   <genérelo, ver abajo>
CARLYS_CORS_ORIGENES https://el-dominio-del-frontend
CARLYS_PERFIL        produccion
CARLYS_ZONA          America/Bogota
CARLYS_LATITUD       <latitud del local>
CARLYS_LONGITUD      <longitud del local>
```
...
`CARLYS_CORS_ORIGENES` con ese dominio exacto. Acepta varios separados por
coma, por si hay un dominio propio además del de Railway:

```
CARLYS_CORS_ORIGENES=https://carlys.up.railway.app,https://carlysrestobar.com.co
```
...
**El volumen llega con dueño `root` y el proceso corre como `carlys`.** La
...
**cambia a `carlys` con `setpriv` antes de ejecutar Java**. Root vive unos
...
|---|---|---|---|
| `DATABASE_URL` | sí | `jdbc:postgresql://localhost:5432/carlys` | Conexión a PostgreSQL |
| `DATABASE_USUARIO` | sí | `carlys` | Usuario de la base |
| `DATABASE_CLAVE` | sí | `carlys` | Clave de la base |
| `CARLYS_JWT_SECRETO` | **sí** | *(ninguno)* | Firma de los tokens. Mínimo 32 caracteres |
| `CARLYS_CORS_ORIGENES` | sí | `http://localhost:5173` | Dominios autorizados, separados por coma |
| `CARLYS_PERFIL` | no | `desarrollo` | `produccion` cambia los registros a JSON |
| `CARLYS_ZONA` | no | `America/Bogota` | Zona del día operativo y del turno |
| `CARLYS_LATITUD` | no | `10.3392264` | Latitud del local, para medir distancias |
| `CARLYS_LONGITUD` | no | `-75.4154494` | Longitud del local |
| `CARLYS_JWT_MINUTOS` | no | `20` | Vida del token de acceso |
| `CARLYS_JWT_DIAS_REFRESCO` | no | `30` | Vida del token de refresco |
| `CARLYS_CLAVE_DEMO` | no | *(vacía)* | Enciende el [modo demostración](#modo-demostración) |
...
| `VITE_URL_SEDE_HERMANA` | no | URL del sitio de la sede de Arjona. Enciende el selector de sede en `/admin`. Hoy va vacía |
...
`CARLYS_CORS_ORIGENES` traiga el dominio exacto del frontend —con `https://` y
...
```bash
cd backend
DATABASE_URL="postgresql://usuario:clave@servidor:5432/carlys" ./scripts/respaldar.sh
```
...
>
> ```
> RESPALDO_S3_BUCKET=carlys-respaldos
> AWS_ACCESS_KEY_ID=...
...
```bash
cd backend
DATABASE_URL="postgresql://..." ./scripts/restaurar.sh respaldos/carlys-20260820T213140Z.sql.gz
```
...
panel de Cloudinary, carpeta `carlys`.
...
> Las coordenadas del local ya son las reales: `10.3392264, -75.4154494`,
...
> restaurante se muda, hay que cambiar los dos sitios: `CARLYS_LATITUD` y
> `CARLYS_LONGITUD` en el backend, y `config.ts` en el frontend.
...
DATABASE_URL=jdbc:postgresql://localhost:5433/carlys \
DATABASE_USUARIO=carlys DATABASE_CLAVE=carlys \
CARLYS_JWT_SECRETO="$(openssl rand -base64 48)" \
CARLYS_CORS_ORIGENES=http://localhost:5173 \
mvn spring-boot:run
...
El 5433 y el 5434 los ocupan otros sistemas hechos sobre esta misma base. Cada
uno tiene su puerto y su nombre de proyecto de Docker para poder
...
```bash
docker build -f backend/Dockerfile -t carlys-backend .
```
...
desde el celular, agregue esa IP a `CARLYS_CORS_ORIGENES` y compile el frontend
