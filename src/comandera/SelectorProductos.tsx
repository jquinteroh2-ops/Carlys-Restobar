        ...
        <div className="flex items-center gap-2 px-3 py-2.5">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="min-h-toque w-full rounded-xl border border-noche-700 bg-noche-850 pl-9 pr-9 text-hueso-100 placeholder:text-noche-500 focus:border-dorado-400 focus:outline-none"
            />
            {busqueda && (
              <button
                type="button"
                ...
                className="absolute right-1 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-lg text-noche-400 hover:text-hueso-100"
              >
              ...
              <button
                ...
                className={`min-h-[40px] shrink-0 rounded-xl border px-3.5 text-sm font-medium transition ${
                  categoriaVisible === categoria.id
                    ? 'border-dorado-400 bg-dorado-500/15 text-dorado-300'
                ...
                <button
                  ...
                  className={`relative flex min-h-[104px] flex-col justify-between rounded-2xl border p-3 text-left transition active:scale-[0.98] ${
                    !item.disponible
                      ...
                      : recienTocado
                        ? 'border-dorado-400 bg-dorado-500/15'
                        : cantidad > 0
                          ? 'border-dorado-400/50 bg-noche-850'
                          : 'border-noche-700 bg-noche-900 hover:bg-noche-850'
                  }`}
                >
                  <span className="pr-7 text-sm font-medium leading-snug text-hueso-100">
                    {item.nombre}
                  </span>
                  <span className="mt-2 flex items-end justify-between gap-2">
                    <span className="text-sm font-semibold tabular-nums text-dorado-300">
                  ...
                  {cantidad > 0 && (
                    <span className="absolute right-2 top-2 flex h-6 min-w-[24px] items-center justify-center rounded-lg bg-dorado-500 px-1 text-xs font-bold text-carbon-950">
                      {cantidad}
                    </span>
                  )}
                </button>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span className="min-w-0">
                <span className="block text-sm font-semibold text-hueso-100">
              ...
              <li key={linea.clave} className="rounded-xl border border-noche-700 bg-noche-850 p-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-hueso-100">{linea.item.nombre}</p>
                    ...
                    {linea.nota && (
                      <p className="mt-0.5 text-xs italic text-dorado-300">{linea.nota}</p>
                    )}
                  </div>
                  <button
                    type="button"
                    ...
                    className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-noche-400 transition hover:bg-noche-700 hover:text-hueso-100"
                  >
