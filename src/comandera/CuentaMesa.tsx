...
export default function CuentaMesa() {
   ...
   * lo emite el sistema contable a partir de esta misma venta; si el cliente lo pide, se
  ...
  const cobrar = async () => {
    ...
    try {
      ...
      // documento fiscal lo emite el sistema contable, y esta pantalla no espera por él.
    ...
    <div className="flex min-h-dvh flex-col bg-noche-950 pb-28">
      <header className="sticky top-0 z-30 flex items-center gap-2 border-b border-noche-800 bg-noche-900/95 px-3 py-2.5 backdrop-blur">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-lg font-semibold text-hueso-100">Cuenta · {etiquetaMesa}</h1>
      ...
      <main className="flex-1 space-y-4 px-3 py-4">
        ...
        <section className="rounded-2xl border border-noche-800 bg-noche-900 p-3">
          <h2 className="mb-2.5 text-sm font-semibold text-hueso-100">Consumo</h2>
          <ul className="space-y-1.5">
            {vigentes.map((item) => (
              <li key={item.id} className="flex items-start justify-between gap-3 text-sm">
                <span className="min-w-0">
                  <span className="text-hueso-100">
            ...
            <ul className="space-y-1.5 text-sm">
              {orden.cargosAdicionales.map((cargo) => (
                <li key={cargo.id} className="flex justify-between gap-3">
                  <span className="min-w-0">
                    <span className="block truncate text-hueso-100">{cargo.nombre}</span>
        ...
        {/* ---------- Totales ---------- */}
        <section className="rounded-2xl border border-dorado-400/30 bg-noche-900 p-3">
          <dl className="space-y-1.5 text-sm">
            ...
            <div className="flex justify-between border-t border-noche-700 pt-2 text-xl font-bold text-hueso-100">
              <dt>Total</dt>
              <dd className="tabular-nums text-dorado-300">{formatoCOP(cuenta.total)}</dd>
            </div>
          </dl>
        </section>
