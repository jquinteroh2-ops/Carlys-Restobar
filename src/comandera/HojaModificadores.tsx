      ...
      <div className="space-y-5">
        ...
        {(item.modificadores ?? []).map((modificador) => (
          <div key={modificador.id}>
            <p className="mb-2 flex items-center gap-2 text-sm font-medium text-hueso-100">
              {modificador.nombre}
              {modificador.obligatorio ? (
                <span className="rounded-md bg-dorado-500/15 px-1.5 py-0.5 text-xs text-dorado-300">
                  Obligatorio
                </span>
              ) : (
                    ...
                    >
                      <span className="min-w-0">
                        ...
                        {opcion.precioAdicional > 0 && (
                          <span className="block text-xs text-dorado-300">
                            +{formatoCOP(opcion.precioAdicional)}
                          </span>
                        )}
                      </span>
                      {activo && <Check className="h-4 w-4 shrink-0 text-dorado-300" aria-hidden />}
                    </button>
                  )
                })}
              </div>
        ...
        <div>
          <p className="mb-2 text-sm font-medium text-hueso-100">Nota para cocina</p>
