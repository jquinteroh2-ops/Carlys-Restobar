 ...
 * esta venta lo emite el sistema contable del restaurante, no la caja.
 */
export function Comprobante({ pago, orden, mesaEtiqueta, meseroNombre }: Props) {
  ...
  return (
    <article className="mx-auto w-full max-w-md rounded-2xl bg-hueso-50 p-5 text-carbon-950 shadow-xl shadow-black/40">
      <header className="border-b border-carbon-950/15 pb-3 text-center">
        <p className="font-marca text-lg tracking-[0.28em]">CARLY’S</p>
        <p className="mt-1 text-xs text-carbon-950/70">
          {RESTAURANTE.direccion} · {RESTAURANTE.ciudad}
        </p>
        <p className="text-xs text-carbon-950/70">{RESTAURANTE.telefono}</p>
      </header>

      <div className="grid grid-cols-2 gap-y-1 border-b border-carbon-950/15 py-3 text-xs text-carbon-950/80">
        <span>Comanda</span>
      ...
      <ul className="space-y-1.5 border-b border-carbon-950/15 py-3">
        {vigentes.map((item) => (
          <li key={item.id} className="flex items-start justify-between gap-3 text-sm">
            <span className="min-w-0">
              ...
              {item.modificadoresSeleccionados.length > 0 && (
                <span className="block text-xs text-carbon-950/60">
      ...
      <dl className="space-y-1.5 py-3 text-sm">
        ...
        <div className="flex justify-between border-t border-dashed border-carbon-950/25 pt-1.5">
          <dt>
            Propina
            <span className="ml-1.5 rounded border border-carbon-950/25 px-1 text-[0.65rem] uppercase tracking-wide">
              Voluntaria
            </span>
          </dt>
        ...
        <div className="flex justify-between border-t border-carbon-950/25 pt-2 text-base font-bold">
          <dt>Total</dt>
          <dd className="tabular-nums">{formatoCOP(pago.total)}</dd>
        </div>
      </dl>

      <div className="border-t border-carbon-950/15 pt-3 text-sm">
        ...
        {pago.divisiones && pago.divisiones.length > 1 && (
          <ul className="mt-1.5 space-y-1 text-xs text-carbon-950/70">
      ...
      <footer className="mt-4 border-t border-carbon-950/15 pt-3 text-center">
        <p className="text-xs text-carbon-950/70">
        ...
        <p className="mt-2 text-[0.65rem] leading-relaxed text-carbon-950/50">
