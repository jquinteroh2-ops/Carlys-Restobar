...
const COLUMNAS: { estado: EstadoColumna; titulo: string; acento: string }[] = [
  { estado: 'pendiente', titulo: 'Pendientes', acento: 'text-hueso-100' },
    ...
    <div className="flex h-dvh flex-col overflow-hidden bg-noche-950">
          ...
          <div className="flex items-center gap-1.5">
            <div className="flex rounded-xl border border-noche-700 bg-noche-850 p-1">
              <button
                ...
                className={`flex h-10 items-center gap-1.5 rounded-lg px-3 text-sm font-medium transition ${
                  !esBar ? 'bg-dorado-500 text-carbon-950' : 'text-noche-300 hover:bg-noche-800'
                }`}
              >
            ...
            <button
              ...
              className={`flex h-toque w-11 items-center justify-center rounded-xl transition ${
                sonido
                  ? 'bg-dorado-500/15 text-dorado-300'
                  : 'text-noche-400 hover:bg-noche-800 hover:text-hueso-100'
              }`}
            >
            ...
            <button
              ...
              className="hidden h-toque w-11 items-center justify-center rounded-xl text-noche-400 transition hover:bg-noche-800 hover:text-hueso-100 sm:flex"
            >
              {pantallaCompleta ? (
      ...
      {!sonido && (
        ...
        >
          ...
          <span className="font-semibold text-dorado-300">Activar aviso sonoro</span>
        </button>
      )}
          ...
          return (
            <section key={columna.estado} className="flex min-h-0 flex-col bg-noche-950">
              <header className="flex items-center justify-between gap-2 border-b border-noche-800 bg-noche-950 px-3 py-2.5">
                ...
                <span className="rounded-lg bg-noche-800 px-2 py-0.5 text-sm font-bold tabular-nums text-hueso-100">
                  {deLaColumna.length}
                </span>
              </header>
                ...
                {grupos.length === 0 ? (
                  ...
                  grupos.map((grupo) => (
                    <div
                      ...
                      className={
                        grupo.length > 1
                          ? 'space-y-2 rounded-2xl border-l-4 border-dorado-400/70 bg-noche-900/40 py-2 pl-2 pr-0.5'
                          : 'space-y-2'
                      }
                    >
                      {grupo.length > 1 && (
                        <p className="px-1 text-xs font-semibold uppercase tracking-wider text-dorado-300">
