...
const ACCION: Record<TurnoEnCocina['estado'], { etiqueta: string; siguiente: EstadoItem; clase: string }> = {
  ...
  listo: {
    ...
    siguiente: 'servido',
    clase: 'bg-noche-700 text-hueso-100 hover:bg-noche-600',
  },
  servido: {
    etiqueta: 'Servido',
    siguiente: 'servido',
  ...
  return (
    <article
      className={`overflow-hidden rounded-2xl border-2 bg-noche-900 transition-colors duration-500 ${
        reciente ? 'animate-destello border-dorado-400' : 'border-noche-700'
      }`}
    >
      <header className="flex items-center justify-between gap-2 border-b border-noche-800 px-3 py-2.5">
        <div className="min-w-0">
          <p className="truncate text-xl font-bold leading-tight text-hueso-100">
            {bloque.mesaEtiqueta}
          </p>
      ...
      {bloque.notas && (
        <p className="flex items-start gap-1.5 border-b border-noche-800 bg-dorado-500/10 px-3 py-2 text-sm text-dorado-200">
              ...
              >
                <span
                  className={`mt-0.5 flex h-7 min-w-[28px] items-center justify-center rounded-lg px-1 text-base font-bold ${
                    listo ? 'bg-estado-listo text-noche-950' : 'bg-noche-700 text-hueso-100'
                  }`}
                >
                  {item.cantidad}
                </span>
                ...
                <span className="min-w-0 flex-1">
                  <span
                    className={`block text-base font-semibold leading-snug ${
                      listo ? 'text-estado-listo line-through' : 'text-hueso-100'
                    }`}
                  >
                    {item.nombre}
                  </span>
                  {item.modificadoresSeleccionados.length > 0 && (
                    <span className="mt-0.5 block text-sm font-medium text-dorado-300">
