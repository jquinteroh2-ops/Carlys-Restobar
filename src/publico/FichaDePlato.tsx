    ...
    <div className="-mx-4 -mt-4">
      ...
      */}
      <div
        ...
        tabIndex={0}
        className="sin-scrollbar flex h-[42dvh] snap-x snap-mandatory overflow-x-auto overscroll-x-contain bg-carbon-950 outline-none"
      >
        {fotos.map((foto, i) => (
          <div
            key={foto}
            ...
            >
              <span
                className={`block h-1.5 transition-all ${
                  i === visible ? 'w-5 bg-dorado-400' : 'w-1.5 bg-hueso-100/30'
                }`}
              />
            </button>
          ))}
...
function Presentacion({ item }: { item: ItemCarta }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {item.descripcion && (
        <p className="text-[0.95rem] leading-relaxed text-hueso-100/70">{item.descripcion}</p>
      )}
      ...
      {enPromocion(item) && (
        <p className="text-sm text-noche-400">
          ...
          {' · '}
          <span className="text-dorado-300">hoy {formatoCOP(precioVigente(item))}</span>
        </p>
      )}
    </div>
  )
  ...
  if (!sePuedePedir) {
    return (
      ...
      >
        ...
        {!item.disponible && (
          <p className="mt-4 border border-noche-700 bg-noche-850 px-3.5 py-3 text-sm text-noche-300">
