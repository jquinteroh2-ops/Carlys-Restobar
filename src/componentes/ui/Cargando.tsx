...
function PantallaDeArranque({ mensaje }: { mensaje?: string }) {
  return (
    <div
      role="status"
      aria-live="polite"
      className="flex min-h-dvh flex-col items-center justify-center gap-6 bg-carbon-950 px-6"
    >
      ...
      <div className="relative flex items-center justify-center">
        <span
          aria-hidden
          className="pointer-events-none absolute h-32 w-32 rounded-full bg-dorado-500/15 blur-2xl"
        />
        <Emblema tamano={76} className="animate-respirar" />
      </div>

      <div className="flex flex-col items-center gap-1.5">
        <span className="font-marca text-lg tracking-[0.3em] text-hueso-100">CARLY’S</span>
      </div>
      ...
      {/* La barra. `overflow-hidden` en el carril y la luz corriéndose dentro:
          ...
          nada en el medio. */}
      <div className="h-px w-44 overflow-hidden rounded-full bg-dorado-500/15">
        <div className="h-full w-1/3 animate-vaiven rounded-full bg-dorado-400/70" aria-hidden />
      </div>

      <span className="font-titulo text-base italic text-hueso-100/60">
