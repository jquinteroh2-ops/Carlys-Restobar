    ...
    <section className="revelar-corto rounded-2xl border border-noche-800 bg-noche-900 p-4">
      <header className="mb-4 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h2 className="text-sm font-semibold text-hueso-100">{titulo}</h2>
      ...
      {datos.map((punto) => (
        <li key={punto.etiqueta} className="group">
          <div className="mb-1 flex items-baseline justify-between gap-3">
            <span className="min-w-0 truncate text-sm text-hueso-100">
              {punto.etiqueta}
              {punto.detalle && (
      ...
      <div className="mb-2 flex h-6 items-center justify-end">
        {enfocada ? (
          <p className="animate-entrada text-xs text-noche-300">
            <span className="font-semibold text-hueso-100">{enfocada.franja}</span> ·{' '}
            {enfocada.texto} · {enfocada.detalle}
          </p>
        ) : (
          <p className="text-xs text-noche-500">
            Hora pico: <span className="font-semibold text-dorado-300">{pico.franja}</span> con{' '}
            {pico.texto}
          </p>
        )}
      </div>
            ...
            <span
              className={`w-full rounded-t-[4px] transition-all duration-300 ${
                activa === punto.hora || (activa === null && punto.hora === pico.hora)
                  ? 'bg-dorado-400'
                  : 'bg-dorado-500/70 group-hover:bg-dorado-400'
              }`}
          ...
          {filas.map((fila, i) => (
            <tr key={i}>
              {fila.map((celda, j) => (
                <td
                  key={j}
                  className={`py-2 ${j > 0 ? 'text-right tabular-nums text-noche-300' : 'text-hueso-100'}`}
                >
                  {celda}
                </td>
              ))}
