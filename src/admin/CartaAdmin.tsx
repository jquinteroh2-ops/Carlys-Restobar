  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 text-xs text-noche-400">
        <span>
          <strong className="text-hueso-100">{carta.length}</strong> productos
        </span>
        {agotados > 0 && (
        ...
        <ul className="grid gap-2 lg:grid-cols-2">
          {visibles.map((item) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p
                  className={`truncate text-sm font-medium ${
                    item.disponible ? 'text-hueso-100' : 'text-noche-400 line-through'
                  }`}
                >
                  {item.nombre}
                </p>
                <p className="truncate text-xs text-noche-500">
                  ...
                  {item.modificadores && item.modificadores.length > 0 && (
                    <span className="ml-1.5 text-dorado-300">
              ...
              <span className="shrink-0 text-right text-sm font-semibold tabular-nums text-hueso-100">
                {enPromocion(item) ? (
                  <>
                    <span className="block text-xs font-normal text-noche-400 line-through">
                      {formatoCOP(item.precio)}
                    </span>
                    <span className="text-dorado-300">{formatoCOP(item.precioPromocional!)}</span>
                  </>
                ) : (
                  formatoCOP(item.precio)
                )}
