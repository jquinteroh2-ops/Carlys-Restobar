...
function TarjetaEntrega({ pedido, trabajando, onEntregar }: PropsTarjeta) {
  ...
  const chip =
    'inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-noche-700 bg-noche-850 px-3 py-3 text-sm text-hueso-100 transition hover:border-dorado-400/50'

  return (
    <article className="rounded-2xl border border-noche-800 bg-noche-900 p-4">
      <header className="mb-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="truncate text-lg font-semibold text-hueso-100">{etiqueta}</p>
          <p className="truncate text-sm text-noche-300">{orden.cliente?.nombre}</p>
        </div>
        <Insignia tono="oro">{espera} min</Insignia>
      </header>

      <p className="mb-2 flex items-start gap-1.5 text-sm text-hueso-100">
      ...
      */}
      <div className="mt-3 flex items-center justify-between rounded-xl border border-noche-700 px-3 py-2">
        ...
        <p className="text-xl font-bold tabular-nums text-hueso-100">{formatoCOP(cuenta.total)}</p>
      </div>
