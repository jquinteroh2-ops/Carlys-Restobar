...
public class ServicioTokens {
  ...
  public ServicioTokens(
      @Value("${carlys.jwt.secreto}") String secreto,
      @Value("${carlys.jwt.minutos-acceso:20}") long minutosAcceso,
      @Value("${carlys.jwt.dias-refresco:30}") long diasRefresco,
      Reloj reloj,
      GeneradorIds ids) {
    ...
    if (bytes.length < 32) {
      throw new IllegalStateException(
          "CARLYS_JWT_SECRETO debe tener al menos 32 caracteres: la firma actual es débil");
    }
