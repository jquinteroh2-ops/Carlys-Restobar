 ...
 * Globalsoft esta instalado localmente. Si es asi, Carly’s —que corre en la
 ...
 */
@Component
@ConditionalOnProperty(name = "carlys.erp.adaptador", havingValue = "rest")
public class FacturacionRest implements FacturacionExterna {
  ...
  public FacturacionRest(
      @Value("${carlys.erp.rest.url:}") String urlBase,
      @Value("${carlys.erp.rest.token:}") String token) {
  ...
  public ResultadoFacturacion emitirDocumento(VentaParaErp venta) {
    if (urlBase.isBlank()) {
      ...
      return ResultadoFacturacion.rechazado(
          "El adaptador REST esta activo pero sin configurar: falta carlys.erp.rest.url", null);
    }
