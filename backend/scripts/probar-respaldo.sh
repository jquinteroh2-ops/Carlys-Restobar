...
# ningún momento: crea `carlys_prueba_restauracion`, la usa y la borra.
#
...
URL="${DATABASE_URL:-postgresql://carlys:carlys@localhost:5433/carlys}"
URL="${URL#jdbc:}"
DESTINO="${DESTINO:-./respaldos}"
BASE_PRUEBA="carlys_prueba_restauracion"
...
ULTIMO="$(ls -1t "${DESTINO}"/carlys-*.sql.gz 2>/dev/null | head -1 || true)"
if [ -z "$ULTIMO" ]; then
